...
class WordFeature:
    ...
    def add_info(cls, document: Document):
        """添加信息"""
        # 1) 添加标题
        cls._add_heading_1(document, "基本信息")
        # 2) 添加段落
        for info_class in InfoBase.__subclasses__():
            info = info_class()
            # 2.1) 添加信息名称
            if not isinstance(info, BasicInfo):
                cls._add_heading_2(document, info.info_name)
            for context in info.to_contexts():
                cls._add_paragraph(document, 20, context)
    ...
    def add_project_experience(cls, document: Document):
        ...
        for project_experience_class in ProjectExperience.__subclasses__():
            ...
            cls._add_heading_2(document, project_name, date_range)
            # 2.2) 添加项目描述
    ...
    @staticmethod
    def _add_heading_1(document: Document, heading_name: str):
        """添加标题"""
    ...
    @staticmethod
    def _add_heading_2(document: Document, heading_name: str, date_range: Tuple[str, str] = None):
        """添加标题日期"""
