...
class ABC02StatementParser(StatementParser):

    def __init__(self, statement_path: str, **kwargs):
        super().__init__("农业银行", statement_path, check_tags=[tag.value for tag in ABC02Tags], **kwargs)
    ...
    def parse_statement(self):
        ...
        for data in ExcelUtil.get_data_list(self.statement_path, tag_row=self.tag_row):
            ...
            statement.trade_datetime = self._format_date(data[ABC02Tags.trade_datetime.value])  # 交易时间
            ...
            statement.abstract = f"{statement.reciprocal_account_name} {data[ABC02Tags.abstract.value]}"  # 摘要
