...
from .CQRCB_receipt_type import CQRCBReceiptType
from ....entity.receipt import Receipt


class CQRCBReceiptType01(CQRCBReceiptType):

    def judge(self) -> bool:
        """判断是否为当前格式"""
        if {"付款人", "收款人", "户名"} < set(self.table.get_row_values(1)):
            return True
        return False
    ...
    def get_receipt(self) -> Receipt:
        ...
        receipt.serial_number = self._get_cell_relative("^流水号$").get_value()  # 流水号
        ...
        receipt.abstract = self._get_cell_relative("^备注$").get_value()  # 摘要
