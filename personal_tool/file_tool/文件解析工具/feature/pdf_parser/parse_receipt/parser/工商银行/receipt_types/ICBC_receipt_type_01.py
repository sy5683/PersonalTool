...
class ICBCReceiptType01(ICBCReceiptType):
    ...
    def judge(self) -> bool:
        """判断是否为当前格式"""
        if {"付款人", "户名", "收款人"} < set(self.table.get_row_values(0)):
            return True
        return False
    ...
    def get_receipt(self) -> Receipt:
        ...
        receipt.date = TimeUtil.format_to_str(self._get_cell_relative("时间戳").get_value())  # 日期
        receipt.receipt_number = self._get_word(r"^电子回单号码[:：](.*?)$")  # 回单编号
