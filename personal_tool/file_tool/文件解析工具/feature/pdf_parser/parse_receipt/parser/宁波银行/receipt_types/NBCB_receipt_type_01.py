...
class NBCBReceiptType01(NBCBReceiptType):
    ...
    def get_receipt(self) -> Receipt:
        ...
        # 获取表格中的用户信息
        account_name = self._get_cell_relative("账号").get_value()
        account_number = self._get_cell_relative("户名").get_value()
        account_bank = self._get_cell_relative("开户银行").get_value()
        # 根据之前备注信息中的用户信息，将表格中的用户信息补充到另一方
        if receipt.payee_account_name or receipt.payee_account_number or receipt.payee_account_bank:
            receipt.payer_account_name = account_name
            receipt.payer_account_number = account_number
            receipt.payer_account_bank = account_bank
        elif receipt.payer_account_name or receipt.payer_account_number or receipt.payer_account_bank:
            receipt.payee_account_name = account_name
            receipt.payee_account_number = account_number
            receipt.payee_account_bank = account_bank
        else:
